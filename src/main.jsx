...
const TIER_INFO = {
  1:{name:'初階', rarity:'普通', frame:'bronze', scale:1, note:'剛踏入這條人生道路，穩定但影響力有限。'},
  2:{name:'二階', rarity:'稀有', frame:'silver', scale:1.45, note:'已具備職業資歷，事件加成更明顯，但風險也開始增加。'},
  3:{name:'傳奇', rarity:'傳奇', frame:'gold', scale:2, note:'深耕同一條職業三次以上，成為足以影響世界的人物。'}
};
function scaleDelta(d={},scale=1){const out={}; Object.entries(d).forEach(([k,v])=>out[k]=Math.round(v*scale)); return out;}
function tieredAchievement(raw, career, tier){
  const [label,title,position,delta,boost,desc]=raw;
  const info=TIER_INFO[tier]||TIER_INFO[3];
  const prefix=tier===1?'':tier===2?'資深':'傳奇';
  const tierTitle=prefix ? `${prefix}${title}` : title;
  const tierLabel=tier===1 ? label : `${info.name}${label}`;
  const risk=tier===1?'風險較低，成長較穩定。':tier===2?'責任提高，相關負面事件可能更有感。':'人生高度曝光，收益大，但代價也可能更大。';
  const special=tier===1?'開啟相關職業事件加成。':tier===2?'同職業事件加成提升，部分事件可能追加收益。':'取得傳奇身份，未來可觸發更高影響力的人生事件。';
  return {label:tierLabel,title:tierTitle,position,delta:scaleDelta(delta,info.scale),boost:scaleDelta(boost,info.scale),desc:`${desc}（${info.name}頭銜）`,career,tier,rarity:info.rarity,frame:info.frame,risk,special,source:`第 ${tier} 次完成「${career}」職業道路`};
}
function getAchievementOptions(career, player){
  const nextTier=Math.min((player.careerExp?.[career]||0)+1,3);
  return (ACHIEVEMENTS[career]||[]).map(raw=>tieredAchievement(raw,career,nextTier));
}
function titleDisplay(p,t){return t.position==='suffix'?`${p.name}${t.title}`:`${t.title}${p.name}`;}
function rarityStars(r){return r==='傳奇'?'★★★★★':r==='稀有'?'★★★★':'★★';}
