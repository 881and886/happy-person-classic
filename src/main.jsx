...
function App(){
  const [stage,setStage]=useState('setup'),[players,setPlayers]=useState(()=>[mkPlayer(0)]),[current,setCurrent]=useState(0),[log,setLog]=useState(['歡迎來到幸福人 Classic 2.2']),[dice,setDice]=useState([]),[moving,setMoving]=useState(false),[prompt,setPrompt]=useState(null),[event,setEvent]=useState(null),[winner,setWinner]=useState(null),[music,setMusic]=useState(false); const audioRef=useRef(null); const cp=players[current];
  useEffect(()=>{const saved=localStorage.getItem('happy-person-v22'); if(saved){try{const s=JSON.parse(saved); if(s.players){setPlayers(s.players);setCurrent(s.current||0);setStage(s.stage||'setup');setLog(s.log||[]);setWinner(s.winner||null)}}catch{}}},[]);
  useEffect(()=>{localStorage.setItem('happy-person-v22',JSON.stringify({players,current,stage,log,winner}));},[players,current,stage,log,winner]);
  ...
  const startGame=()=>{if(players.some(p=>!p.targetLocked))return alert('請先鎖定每位玩家的幸福目標'); setStage('game'); addLog('遊戲開始：外圈雙骰，內圈單骰。');};
  const reset=()=>{if(confirm('確定重新開始？目前進度會清除。')){localStorage.removeItem('happy-person-v22');location.reload();}}; const nextTurn=()=>{setCurrent(i=>(i+1)%players.length);setDice([]);setPrompt(null);setEvent(null);};
  const finishTurn=()=>setTimeout(()=>{const p=players[current]; if(p&&checkWin(p)){setWinner(p);setStage('end');addLog(`${p.name} 達成幸福目標，遊戲結束！`)}else nextTurn();},80);
...
${deltaText(ach.delta)}`);};
  ...
  return <div className="app"><audio ref={audioRef} src="/bgm.wav"/><header><h1>幸福人 Classic <span>2.2 人生頭銜版</span></h1><div><button onClick={()=>setMusic(!music)}>{music?'🔊 音樂開':'🔇 音樂關'}</button><button onClick={reset}>重新開始</button></div></header>{stage==='setup'&&<Setup players={players} setCount={setCount} updatePlayer={updatePlayer} startGame={startGame}/>} {stage==='game'&&<main className="game"><section className="left"><Recent log={log}/><Board players={players}/><CareerMaps players={players}/></section><aside><TurnPanel cp={cp} dice={dice} moving={moving} locked={!!prompt||!!event} doRoll={doRoll} useExp={useExp} useChance={useChance} sellChance={sellChance} equipTitle={equipTitle} clearTitle={clearTitle} declareBankrupt={declareBankrupt} retireVacation={retireVacation}/><Players players={players} current={current}/></aside></main>} {stage==='end'&&<End winner={winner} players={players} reset={reset}/>} {prompt&&<Prompt prompt={prompt} cp={cp} enterCareer={enterCareer} setPrompt={setPrompt} nextTurn={nextTurn} useChance={useChance} retireVacation={retireVacation} confirmSell={confirmSell} chooseAchievement={chooseAchievement}/>} {event&&<EventModal event={event} confirm={confirmEvent}/>}</div>;
}
